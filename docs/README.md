...
> **提示**: 建议在项目的 `screenshots/` 目录下添加功能截图，方便展示。
...
### Q1: 如何切换数据库？
**A:** 使用不同的启动命令：
- SQLite: `python run_meituan.py`
- MySQL: `python run_meituan.py mysql`

### Q2: 忘记管理员密码怎么办？
**A:** 重新初始化数据库：`python run_meituan.py init`

### Q3: 图片上传失败？
**A:** 检查 `app/static/images/` 目录的写入权限。

### Q4: 端口5000被占用？
**A:** 在 `run_meituan.py` 中修改端口号。

### Q5: 如何添加新餐厅？
**A:** 使用管理员账号登录后，在管理后台添加。
